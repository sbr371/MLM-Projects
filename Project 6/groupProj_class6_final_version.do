...
* In other words, what fraction of the between-school variance in the first model 
*   is 'explained' by the addition of a student random effect?
...
*generate the BLUPs for the random effects and examine whether 
* the independence between zeta0 and zeta1 is REFLECTED in a scatterplot of these two sets of effects.

*compute V_S(year=0) and V_S(year=1).  Since there are only two years, 
* this is a form of heteroscedasticity in the random effects.
...
* Is the correl. signif.?

*compute V_S(year=0) and V_S(year=1) for this new model (your formula should include covariance terms).  
* Is this result (and thus model) more consistent with the separate grade analysis?  
